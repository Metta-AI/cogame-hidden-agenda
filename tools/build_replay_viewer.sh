...
# index.html). `coworld build` refuses to package a source replay-viewer bundle
# unless this file is os.X_OK, so it ships mode 100755.
set -euo pipefail
...
# parent BEFORE resolving it — the inherited bug every fork of paintbot's hook
# carries (ecos, 2026-08-23).
...
image_tag="coworld-hidden-agenda-replay-viewer-build:$$"
container_id=""
cleanup() {
...
mkdir -p "${output_dir}"
docker cp \
  "${container_id}:/workspace/hidden_agenda/replay-viewer/dist/." \
  "${output_dir}"

test -s "${output_dir}/index.html"
test -s "${output_dir}/hidden_agenda_replay.wasm"
test -s "${output_dir}/static_replay.js"
test -s "${output_dir}/static_replay_worker.js"
test -s "${output_dir}/broadcast_core.js"
grep -q 'data-replay-loaded' "${output_dir}/static_replay.js"
echo "hidden-agenda replay viewer bundle: ${output_dir}"
