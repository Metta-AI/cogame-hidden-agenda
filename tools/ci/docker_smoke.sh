...
# Substitute: hidden-agenda, coworld-hidden-agenda, 5.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-hidden-agenda:ci)
#   SMOKE_SLUG                 game slug                        (hidden-agenda)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/hidden-agenda)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/hidden-agenda-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-hidden-agenda:ci}}"
slug="${SMOKE_SLUG:-hidden-agenda}"
