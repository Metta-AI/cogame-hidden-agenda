...
function stageNote() {
  ...
  try {
    var length = Module._hidden_agenda_stage_len ? Module._hidden_agenda_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._hidden_agenda_stage_ptr();
...
function runtimeError() {
  var length = Module._hidden_agenda_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._hidden_agenda_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._hidden_agenda_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._hidden_agenda_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._hidden_agenda_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._hidden_agenda_load_replay(pointer, length);
    });
...
function applyInputNow() {
  ...
  try {
    if (Module._hidden_agenda_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({ type: 'inputApplied' });
  } catch (error) {
    reportFailure(error);
  }
}
...
importScripts('./wire_constants.js', './broadcast_core.js', './hidden_agenda_replay.js');
